...
static void BM_lmv_2d_vectorized(benchmark::State& state)
{
  ...
  for (auto _ : state) {
    // This code gets timed
    lmv_2d_vectorized<W>(n, k, Su, Smean);
  }
}

template <int W>
static void BM_lmv_2d_vectorized_buffered(benchmark::State& state)
{
  // Perform setup here
  size_t n = state.range(0);
  size_t k = state.range(1);
  ...
  for (auto _ : state) {
    // This code gets timed
    lmv_2d_vectorized_buffered<W>(n, k, Su, Smean);
  }
}
...
template <int MI, int MJ, int W>
static void BM_lmv_2d_vectorized_buffered_blocked(benchmark::State& state)
{
  // Perform setup here
  size_t n = state.range(0);
  size_t k = state.range(1);
  ...
  for (auto _ : state) {
    // This code gets timed
    lmv_2d_vectorized_buffered_blocked<MI, MJ, W>(n, k, Su, Smean);
  }
}

BENCHMARK(BM_lmv_2d)
  ->ArgsProduct({{64, 128, 256, 512, 1024, 2048, 4096, 8192}, {1, 3, 5, 7}})
  ->Unit(benchmark::kMillisecond);
BENCHMARK_TEMPLATE(BM_lmv_2d_blocked, 1024, 1024)
  ->ArgsProduct({{64, 128, 256, 512, 1024, 2048, 4096, 8192}, {1, 3, 5, 7}})
  ->Unit(benchmark::kMillisecond);
BENCHMARK_TEMPLATE(BM_lmv_2d_vectorized, 4)
  ->ArgsProduct({{64, 128, 256, 512, 1024, 2048, 4096, 8192}, {1, 3, 5, 7}})
  ->Unit(benchmark::kMillisecond);
BENCHMARK_TEMPLATE(BM_lmv_2d_vectorized_blocked, 4, 256, 4)
  ->ArgsProduct({{64, 128, 256, 512, 1024, 2048, 4096, 8192}, {1, 3, 5, 7}})
  ->Unit(benchmark::kMillisecond);
BENCHMARK_TEMPLATE(BM_lmv_2d_vectorized_buffered, 4)
  ->ArgsProduct({{64, 128, 256, 512, 1024, 2048, 4096, 8192}, {1, 3, 5, 7}})
  ->Unit(benchmark::kMillisecond);
BENCHMARK_TEMPLATE(BM_lmv_2d_vectorized_buffered_blocked, 4, 256, 4)
  ->ArgsProduct({{64, 128, 256, 512, 1024, 2048, 4096, 8192}, {1, 3, 5, 7}})
  ->Unit(benchmark::kMillisecond);
